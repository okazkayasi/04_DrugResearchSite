...
const chartBuilder = (olddata, field) => {
	const data = []
	olddata.forEach((item) => {
		...
		const stRevenue = (item.Revenue + "").startsWith("$")
			...
			: item.Revenue
		newItem.Revenue = parseInt(stRevenue)

		newItem.xVals =
			parseInt(item[field].slice(0, item[field].length - 1)) / 100
		data.push(newItem)
	})
	...
	svg.append("text")
		...
		.style("font-size", 28)
		.style("text-anchor", "middle")
		.text("Worldwide revenue, 2020 (log scale)")
	...
	svg.append("g").call(
		d3.axisLeft(y).ticks(3, (d) => "$" + d3.format(",.0f")(d) + "M")
	)
	...
	function mouseOverHandler(that, d) {
		...
		var tooltip2 = d3
			...
			.html(
				`
        <div class='tooltip-colored'>
          <div class='tooltip-header-row'>
            <h4 class='tooltip-revenue'><b>Drug Name:</b>${
				" " + d.Drug_Name
			}</h4>
            <h4 class='tooltip-revenue'><b>2020 Revenue:</b>${
				" $" +
				d.Revenue.toLocaleString("en", { useGrouping: true }) +
				"M"
			}</h4>
            <h4 class="tooltip-revenue">
              <b>AE's:</b> ${" " + d.xVals + "%"}
            </h4>
          </div>
          <a href='${d.Label_URL}' target='_blank'> 
            FDA label(PDF)
          </a>
	...
	svg.append("g")
		.selectAll("circle")
		.data(data.filter((d) => d.REMS !== "Yes" && d.Black_Box !== "Yes"))
		.enter()
		.append("circle")
		.attr("cx", (d) => x(d.xVals))
		.attr("cy", (d) => {
